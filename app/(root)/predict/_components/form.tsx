...
export const FormInput = ({
  ...
  setResult: Dispatch<SetStateAction<undefined | number>>;
  setProfile: Dispatch<SetStateAction<undefined | { name: string }>>;
  loading: { loading: boolean; setLoading: Dispatch<SetStateAction<boolean>> };
}) => {
  ...
  async function onSubmit(values: z.infer<typeof backupSchema>) {
    ...
    await fetch(`${process.env.NEXT_PUBLIC_SERVER}/predict`, {
      method: "POST",
  ...
  return (
    <Form {...form}>
      ...
      >
        ...
        <Button disabled={loading.loading} type="submit" className="mt-8">
          Check
        </Button>
      </form>
    </Form>
