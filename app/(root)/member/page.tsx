    ...
    <main className="w-full gap-8 md:h-full flex flex-col justify-start md:justify-center items-center px-4 md:px-6">
      ...
      <div className="flex flex-wrap justify-center items-center gap-12">
        {members.map((member: Member, index: number) => {
          return (
            <m.div animate={{ y: [100, 0], opacity: [0, 1] }} transition={{ ease: "anticipate", duration: 2, delay: index * 0.5 }} key={index} className="flex flex-col justify-start items-center gap-4">
              <div className="w-52 border aspect-square rounded-full flex justify-center items-center">
                {member.profile && <Image src={member.profile} alt="profile" width={1000} height={1000} className="w-full h-full object-cover" />}
