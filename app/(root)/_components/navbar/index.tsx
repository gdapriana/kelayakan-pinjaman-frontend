...
import {AlignJustify, CircleHelp, FlaskRound, HandCoins, Home, Users} from "lucide-react";
    ...
    >
      ...
      <div className="md:flex ms-auto hidden justify-center items-center gap-6">
        {navigations.map((nav: Navigation, index: number) => {
          return (
            <Link key={index} className="text-lg text-muted-foreground flex justify-center items-center gap-2 hover:text-primary font-medium" href={nav.url}>
          ...
          {navigations.map((nav: Navigation, index: number) => {
            return (
              <DropdownMenuItem key={index}>
                <Link className="text-lg text-muted-foreground hover:text-primary font-medium" href={nav.url}>{nav.name}</Link>
              </DropdownMenuItem>
            )
          })}
        </DropdownMenuContent>
